...
class Email extends SugarBean {
	...
	public function email2saveAttachment()
	{
		...
		//masip
		for ($i=0; $i < count($fileName, COUNT_RECURSIVE); $i++) { 

			$email_uploads[$i] = "modules/Emails/{$GLOBALS['current_user']->id}";

			$guid = create_guid();
			$name_list = $fileName[$i];
			if($upload->final_move($guid, $i)) 
			{				
				$GLOBALS['log']->debug("Email Attachment [$name_list[$i]]");
				
				copy("upload://$guid", sugar_cached("$email_uploads[$i]/$guid"));

				$lista[$i] =  array(
					'guid' => $guid,
					'name' => $GLOBALS['db']->quote($fileName[$i]),
					'nameForDisplay' => $fileName[$i]
				);
