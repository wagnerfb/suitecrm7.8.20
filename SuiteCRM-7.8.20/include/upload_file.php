...
class UploadFile
{
	...
	{
        //masip 
        for ($i=0; $i < count($_FILES_element['name'], COUNT_RECURSIVE); $i++) { 
            # code...
            $filename = $_FILES_element['name'][$i];
            $filetype = isset($_FILES_element['type'][$i]) ? $_FILES_element['type'][$i] : null;
            $file_ext = pathinfo($filename[$i], PATHINFO_EXTENSION);
            //$file_ext = "";
            ...
            if ($filetype && !$recheckMime) {
                ...
                $mime = mime_content_type( $_FILES_element['tmp_name'][$i] );
            } elseif ($is_image) {
                $info = getimagesize($_FILES_element['tmp_name'][$i]);
                if ($info) {
                    $mime = $info['mime'];
                }
            } elseif (function_exists('ext2mime')) {
                $mime = ext2mime($filename[$i]);
            }
        }
	...
	function create_stored_filename()
	{
        ...
        //masip
        for ($i=0; $i < count($stored_file_name, COUNT_RECURSIVE); $i++) { 

            $this->file_ext = pathinfo($stored_file_name[$i], PATHINFO_EXTENSION);
        }
    ...
    //massip
	function final_move($bean_id,$Intervalo)
	{
	    $destination = $bean_id;
        ...
        if($this->use_soap) {
			...
			if(!UploadStream::move_uploaded_file($_FILES[$this->field_name]['tmp_name'][$Intervalo], $destination)) {
